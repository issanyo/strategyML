...
class SimpleStrategy:
    ...
    def predict(self, state):
        ...
        if fees_last_quartile < 0.12:

            if not currently_inside_range and (deviation_last_quartile > 0.6 or (0.05 < p_reward < 0.65 and (
                    p_above_curr_range * p_below_curr_range > 1e-05 or (
                    p_reward_last_quartile > 0.4 and p_outside_curr_range > 0.8)))):
                # increment, outsiders up and down
                print("SimpleStrategy if#1")
                action = 1
            elif p_reward > 0.99 and p_reward_last_quartile > 0.99 and deviation < 0.2:
                # decrement, majourity of values are inside
                print("SimpleStrategy if#2")
                action = 2

            # check if we are still inside the current range
            elif deviation_last_quartile <= 1.0 and not currently_inside_range and p_reward_last_quartile < 0.2: #mean > curr_range and
